    ...
    fetch('http://localhost:5000/api/stations')
      ...
      .catch(err => {
          ...
          fetch('http://localhost:5000/api/daynight/list')
            ...
            .then((filenames: string[]) => {
                if (!filenames || filenames.length === 0) {
                    toast.warning("No station plots found. Run your batch jobs first."); setStations([]); return;
                }
                const parsedStations = filenames.map(filename => {
                    const id = filename.replace('station_', '').replace('.png', '');
                    return { id: id, name: `Monitoring Station ${id}` };
            ...
            <div className="max-w-md">
              ...
              {isStationsLoading ? ( <Loader2 className="h-4 w-4 animate-spin" /> ) : (
                <Select value={selectedStation || ''} onValueChange={handleStationChange}>
                  ...
                  <SelectContent className="max-h-64">
                    {stations.map((station) => (
                      // --- (MODIFIED) Show only station name in dropdown ---
                      <SelectItem key={station.id} value={station.id}>
                        {station.name}
                      </SelectItem>
                    ))}
          ...
          <Tabs defaultValue="day-night" className="space-y-8">
            ...
            <TabsContent value="day-night" className="space-y-6 animate-fade-in">
              <Card className="p-8 shadow-medium border-2">
                 ...
                 <p className="text-muted-foreground mb-6"> Average values for {selectedStation ? stations.find(s => s.id === selectedStation)?.name : '...'} (#{selectedStation}) </p>
                 <div className="flex justify-center items-center min-h-[400px] bg-muted/30 rounded-lg mb-6">
            ...
            <TabsContent value="correlation" className="space-y-6 animate-fade-in">
              <Card className="p-8 shadow-medium border-2">
                 ...
                 <p className="text-muted-foreground mb-6"> {correlationMethod.charAt(0).toUpperCase() + correlationMethod.slice(1)} correlation matrix for {selectedStation ? stations.find(s => s.id === selectedStation)?.name : '...'} (#{selectedStation}). </p>

                 {/* --- (MODIFIED) Heatmap container (full width now) --- */}
                 ...
                 {/* --- (MODIFIED) Insights Box moved below and styled --- */}
                                ...
                                <ul className="space-y-1.5 text-sm text-muted-foreground">
                                  {correlationInsights.positive.map((pair, idx) => (
                                    <li key={`pos-${idx}`} className="flex justify-between items-center">
                                      <span> <span className='font-medium text-foreground'>{formatParamName(pair.param1)}</span> & <span className='font-medium text-foreground'>{formatParamName(pair.param2)}</span> </span>
                                      <span className="font-mono text-xs bg-green-100 text-green-800 px-1.5 py-0.5 rounded"> {pair.correlation.toFixed(2)} </span>
                                    </li>
                                  ))}
                                </ul>
                                ...
                                <ul className="space-y-1.5 text-sm text-muted-foreground">
                                  {correlationInsights.negative.map((pair, idx) => (
                                     <li key={`neg-${idx}`} className="flex justify-between items-center">
                                       <span> <span className='font-medium text-foreground'>{formatParamName(pair.param1)}</span> & <span className='font-medium text-foreground'>{formatParamName(pair.param2)}</span> </span>
                                       <span className="font-mono text-xs bg-red-100 text-red-800 px-1.5 py-0.5 rounded"> {pair.correlation.toFixed(2)} </span>
                                    </li>
                                  ))}
                                </ul>
                             ) : ( <p className="text-sm text-muted-foreground italic">None found.</p> )}
