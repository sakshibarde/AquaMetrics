...
const Analysis = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/anomaly-heatmap')
      ...
      .then(data => {
        setHeatmapData(data);
        generateAnomalyInsights(data); // Generate new insights structure
      })
      .catch(err => toast.error("Failed to load anomaly heatmap."))
      .finally(() => setIsHeatmapLoading(false));
  }, []); // Runs once
  ...
  const generateAnomalyInsights = (plotData: any) => {
    ...
    if (Object.keys(stationCounts).length > 0) {
      const topStationId = Object.entries(stationCounts).sort(([,a], [,b]) => b - a)[0][0];
      stationWithMost = { station: topStationId, count: stationCounts[topStationId] };
    }
    ...
    if (Object.keys(parameterCounts).length > 0) {
      const topParam = Object.entries(parameterCounts).sort(([,a], [,b]) => b - a)[0][0];
      parameterWithMost = { parameter: topParam, count: parameterCounts[topParam] };
    }
