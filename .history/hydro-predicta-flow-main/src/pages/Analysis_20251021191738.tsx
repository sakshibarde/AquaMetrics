...
<TabsContent value="correlation" className="space-y-6 animate-fade-in">
              <Card className="p-8 shadow-medium border-2">
                 ...
                 {/* Method Selection */}
                 <div className="mb-6 max-w-lg"> {/* Slightly wider max-width */}
                    <Label className="text-sm font-medium mb-2 block">Correlation Method</Label>
                    <RadioGroup value={correlationMethod} onValueChange={(value) => setCorrelationMethod(value as CorrelationMethod)} className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                        <div className="flex items-center space-x-2"> <RadioGroupItem value="pearson" id="pearson" /> <Label htmlFor="pearson" className="cursor-pointer">Pearson</Label> </div>
                        <div className="flex items-center space-x-2"> <RadioGroupItem value="spearman" id="spearman" /> <Label htmlFor="spearman" className="cursor-pointer">Spearman</Label> </div>
                        <div className="flex items-center space-x-2"> <RadioGroupItem value="kendall" id="kendall" /> <Label htmlFor="kendall" className="cursor-pointer">Kendall</Label> </div>
                    </RadioGroup>
                    {/* --- ADD THIS EXPLANATION DIV --- */}
                    <div className="mt-3 pl-1 text-xs text-muted-foreground space-y-1 border-l-2 border-border/50 ml-1">
                        <p className="pl-3">• **Pearson:** Measures linear relationships (how well data fits a straight line).</p>
                        <p className="pl-3">• **Spearman:** Measures monotonic relationships (how well data follows a general trend, even non-linear).</p>
                        <p className="pl-3">• **Kendall:** Measures ordinal association (similarity of rankings between variables).</p>
                        <p className="pl-3 pt-1">*(Spearman is often a good general-purpose choice)*</p>
                    </div>
                    {/* --- END ADDED DIV --- */}
                 </div>
