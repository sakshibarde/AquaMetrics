...
import Header from '@/components/Header';
import { Card } from '@/components/ui/card';
import { MapPin, Info, Loader2 } from 'lucide-react'; // Import Loader2
import QualityBadge from '@/components/QualityBadge';
...
interface Station {
  id: number; // Use number as returned by backend
  name: string;
  lat: number;
  lng: number;
  quality: QualityLevel;
  location: string; // This will be the state name from the CSV
}

const MapView = () => {
  const [stations, setStations] = useState<Station[]>([]); // Start empty
  const [selectedStation, setSelectedStation] = useState<Station | null>(null);
  const [isLoading, setIsLoading] = useState(true); // Loading state

  // Effect to load stations from backend
  useEffect(() => {
    setIsLoading(true);
    fetch('http://localhost:5000/api/stations') // Fetch from your new endpoint
      .then(res => {
        if (!res.ok) {
      ...
      .then((data: Station[]) => {
        if (!data || data.length === 0) {
          ...
          setStations([]); // Ensure stations is an empty array
          return;
        }
        setStations(data);
        // Select the first station by default if data exists
        if (data.length > 0) {
      ...
      .catch(err => {
        ...
        setStations([]); // Ensure stations is an empty array on error
      })
  ...
  const getMarkerColor = (quality: QualityLevel) => {
    switch (quality) {
      ...
      case 'Bad': return '#f97316';
      case 'Very Bad': return '#dc2626';
      default: return '#6b7280'; // Default gray for unknown/error
    }
  };
      ...
      <div className="pt-24 pb-16 px-4">
        <div className="container mx-auto max-w-7xl">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <Card className="p-8 shadow-medium border-2 h-[600px] relative overflow-hidden">
                {/* Placeholder Map - Replace with actual map component later */}
                <iframe
                  // Update src based on selectedStation or show India overview
                  src={selectedStation
          ...
          {selectedStation && (
            <Card className="mt-8 p-8 shadow-medium border-2 animate-scale-in">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-muted/50 rounded-lg">
                  <p className="text-sm text-muted-foreground mb-1">Last Updated</p>
                  {/* Note: Backend doesn't provide this, using frontend time */}
                  <p className="font-medium">{new Date().toLocaleString()}</p>
                </div>
              </div>
            </Card>
