...
const generateAnomalyInsights = (plotData: any) => {
  ...
  console.log("Raw plotData received:", plotData);
  ...
  // Assign and log the core data arrays
  const z = plotData.data[0].z as number[][]; // Counts
  const parameters = plotData.data[0].x as string[]; // Parameters
  const stations = plotData.data[0].y as string[]; // Stations (IDs as strings)
  ...
  if (stations.length === 0 || parameters.length === 0 || z.length === 0 || z[0].length === 0) {
     console.warn("generateAnomalyInsights: One or more data arrays (z, x, y) are empty.");
     setAnomalySummary({ // Set an empty summary object instead of null
       totalAnomalies: 0,
       stationWithMost: null,
       parameterWithMost: null,
       topHotspots: [],
     });
     return;
  }
  ...
  try { // Add a try...catch block around calculations
    for (let i = 0; i < stations.length; i++) {
      let currentStationTotal = 0;
      // Ensure z[i] exists and has the expected length
      if (!z[i] || z[i].length !== parameters.length) {
         console.error(`generateAnomalyInsights: Row z[${i}] has unexpected length or is missing.`);
         continue; // Skip this station row
      }
      for (let j = 0; j < parameters.length; j++) {
        const count = z[i][j];
        // Ensure count is a number
        if (typeof count !== 'number' || isNaN(count)) {
           console.warn(`generateAnomalyInsights: Invalid count value at z[${i}][${j}]:`, count);
           continue; // Skip this cell
        }
        if (count > 0) {
          totalAnomalies += count;
          currentStationTotal += count;
          parameterCounts[parameters[j]] = (parameterCounts[parameters[j]] || 0) + count;
          hotspots.push({ station: stations[i], parameter: parameters[j], count: count });
        }
      }
      stationCounts[stations[i]] = currentStationTotal;
    }
    ...
    // Find station with most anomalies
    let stationWithMost: { station: string; count: number } | null = null;
    if (Object.keys(stationCounts).length > 0) {
      const topStationEntry = Object.entries(stationCounts).sort(([,a], [,b]) => b - a)[0];
       if (topStationEntry && topStationEntry[1] > 0) { // Ensure count > 0
          stationWithMost = { station: topStationEntry[0], count: topStationEntry[1] };
       }
    }
    ...
    if (Object.keys(parameterCounts).length > 0) {
       const topParamEntry = Object.entries(parameterCounts).sort(([,a], [,b]) => b - a)[0];
       if (topParamEntry && topParamEntry[1] > 0) { // Ensure count > 0
         parameterWithMost = { parameter: topParamEntry[0], count: topParamEntry[1] };
       }
    }
     console.log("Parameter with most:", parameterWithMost);


    // Sort hotspots and take top 5
    hotspots.sort((a, b) => b.count - a.count);
    const topHotspots = hotspots.slice(0, 5);
    console.log("Top 5 hotspots:", topHotspots);


    // Set the final summary state
    const finalSummary = {
      totalAnomalies,
      stationWithMost,
      parameterWithMost,
    ...
    setAnomalySummary(finalSummary);
     ...
     setAnomalySummary(null); // Set to null on error
  }
};
