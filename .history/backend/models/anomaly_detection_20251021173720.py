        ...
        if not isinstance(z_data, list) or (len(z_data) > 0 and not isinstance(z_data[0], list)):
             ...
             if isinstance(z_data, np.ndarray):
                 ...
                 try:
                     ...
                     print(f"   ⚠️ WARNING: Could not convert 'z' data of type {type(z_data)} to standard list. Frontend might fail.")
                     fig_dict['data'][0]['z'] = [] # Fallback to empty list


    print(f"✅ Saving heatmap JSON (with standard list 'z') to: {OUTPUT_JSON}")
